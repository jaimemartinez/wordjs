...
*   **`createProxyServer()`** — builds the `http-proxy` server with `{ xfwd: true, changeOrigin: true }`. With `changeOrigin`, the upstream receives the target's Host, while the gateway pins the **original client Host** into `X-Forwarded-Host`. The backend's CSRF check reads `X-Forwarded-Host` and requires an **exact origin match** against the configured site URL (or against that pinned host itself) — and the origin check is only **half** of the gate: a cookie-authenticated `POST`/`PUT`/`PATCH`/`DELETE` must additionally present an `X-CSRF-Token` header byte-equal to its `wjs_csrf` cookie, or the backend answers `403 rest_csrf_token`. A Bearer-authenticated caller is exempt from that token half — it has no ambient cookie to ride — and may omit `Origin`/`Referer` entirely; a cookie request that sends neither header is refused `403 rest_csrf_invalid`. Forwarding the client-facing Host is what keeps a proxied write matching — `changeOrigin: true` hands the upstream the *target's* Host, so without `X-Forwarded-Host` the backend's trusted host would be the internal upstream. **Pinning** it (`gateway/src/index.js` overwrites `x-forwarded-host` with the listener's own `Host` and deletes `x-forwarded-server`) is a separate, security reason: `http-proxy`'s `xfwd` would otherwise keep a client-supplied value, and a remote attacker could forge it to satisfy the origin half of the gate.
