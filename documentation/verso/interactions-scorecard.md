...
|---|---|---|---|
| **P1** | **Control completeness & honesty** | Panel UI for repeat/infinite/ping-pong (`repeat`/`alt`), `click.toggle`, `load.delay`, view/scrub **range editor** (author-language over `IxRange`); compiler warnings where options are silently ignored today (repeat/alt on scrub & hover-2-step, stagger on self/block); JS built-chunk byte-budget test; Firefox page-scrub progress parity; canvas collision-suffix fidelity fix | The model already supports all of it — highest value per line of code, and it converts silent lies into honest warnings (method lesson: guarantee properties where they can't be dodged) |
| **P2** | **Easing engine** | Custom cubic-bezier per step; **spring/bounce/elastic compiled to CSS `linear()`** (zero-JS physics — a genuine first among these competitors); extended named-ease table; accessible graphical curve editor (keyboard + numeric, not mouse-only) | Multiplies the perceived quality of every existing and future interaction; compile-side is cheap, Baseline support is universal |
| **P3** | **Property expansion** | rotateY, z + perspective control, skewX/skewY, scaleX/scaleY, transform-origin (closed enum), extended filters, **color/background/border via integer-encoded RGB**, clip directions; fuzz gate extended to prove the closed list still holds | Closes the biggest visible breadth gap vs all three, without one reflow property |
| **P4** | **Stagger 2.0 + responsive gating** | `sibling-index()` native stagger (nth-child fallback), exact `from:center/edges`, total-time mode via `sibling-count()`, author-declared grid mode; per-breakpoint enable (closed enum → `@media` wrap) | Signature "wow" of modern motion; the platform primitives just landed cross-browser (Firefox 154: 2026-08-18) |
| **P5** | **Multi-track editing + visual step strip** | Panel edits all 3 tracks (per-track target/timing/steps), accessible horizontal timeline strip, block-scoped replay | The data model has always supported it; unlocks layered compositions (parallax bg + rising fg + staggered children) |
