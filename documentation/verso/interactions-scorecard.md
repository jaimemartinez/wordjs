...
| **P6** | **Pointer trigger (quarantined runtime)** — **SHIPPED 2026-08-16** | `{on:"pointer"}` positions animations with the cursor: area self (tilt) or page (parallax), exponential chase smoothing (fixed-dt, deterministic, node-tested), **per-track axis** so two tracks compose 2D tilt via the P5 animation list, center-rest contract (step 50 = neutral), IO-gated (off-screen ignores the cursor), single shared rAF, cleanup removes the listener. Lives inside the WAAPI chunk: 1634 B gz of 4096 total, loaded only when used; inert under reduced-motion and on touch. Browser-verified on both surfaces: canvas computed `y 23.94→0 / opacity 0→1` across the cursor travel; public page chasing the cursor with the smoothing visible | The one trigger class CSS cannot express and IX3's 2026 flagship; our per-page conditional loading beats their site-wide GSAP |
| **P7** | **Preset library & parametrization** — **SHIPPED 2026-08-16** | System catalog 16 → 26 (taxonomy pinned by test: 18 entrance / 6 scroll / 2 pointer): linear() physics entrances, 3D flip, skewed slide, children cascade + grid ripple, pointer tilt/parallax (composed by self+children NESTING — same-element tracks fight over transform, and the P5 overlap warning now checks CSS property families, not model keys), directional scroll reveal and color wash. Per-block **intensity** (amt ×0.1–3) scales spatial distance-from-neutral, baked at compile time into the hash; it and the device gating are block-level keys preserved by every spec rebuild (withSpecExtras, revert-red tested — the old writeTrigger silently dropped them). Admin: live usage counts on cards and a real preview (plays the draft, scrubs clockless triggers). Browser-verified: slider ×2 → live WAAPI keyframe 24px → 48px; card shows «Lo usa 1 bloque»; preview running mid-flight | Compounds P2–P6 into one-click author value; presets are already our architectural killer feature |
...
| **P11** | **SHIPPED** (core `e141f02`) | `{on:"event", name}` closed slug `[a-z0-9][a-z0-9-]{0,40}` (1–41 chars, first char alphanumeric; the `wjs:ix:` prefix is added by the code, never by the author), runtime listens `wjs:ix:<name>` on the document, latch/toggle like click; invalid names normalize away (the trigger falls back to the default `view` + `once`) |
...
| **Adversarial sweep** | **REMEDIATED** | A 22-agent review of the whole cycle diff (4 lenses, 2 refuters per finding) confirmed 4 real defects, all fixed with revert-red tests: (1) a step dragged onto its neighbour's `at` was DELETED by the normalizer's dedupe — `setStepAt` now bounds intermediate steps to (prev+1, next−1), closing drag, arrows and the numeric field in one place (browser-verified: a drag past the neighbour pins at neighbour−1 and the neighbour survives); (2) `setScrubSrc`/`setRangeEdge`/`resetRange` rebuilt the trigger without `smooth`, silently reverting P10 — all three now carry it; (3) `draw` × intensity>1 pushed `stroke-dashoffset` outside 0..1 and the dash pattern wrapped (hidden stroke rendered fully drawn) — one shared `drawOffset` saturates 0..100 for both CSS and WAAPI backends; (4) `event`+`block` target compiled to a broken "resolved by runtime" promise — now declared unsupported with its own compiler warning. Plus: activating a track from a passive lane dropped keyboard focus to `<body>` (the lane swaps element type on selection) — focus now lands on the active lane's label, browser-verified |
...
| **C4** | **SHIPPED** (`4d54af8`) | RTL mirroring via `calc(var(--wjs-ix-dir,1) * …)` in CSS and a pre-baked `kfRtl` set for WAAPI (a `var()` inside `Element.animate()` never resolves — it would kill the animation in silence, precisely on the fallback browser). Theme colours travel in their own step key `tint` (closed list of 9 tokens × 3 colour properties) and emit `var(--wjs-color-…)`. Browser-verified on the served stylesheet: with the site re-coloured live, the tinted animation followed to `rgb(255,0,0)→rgb(0,255,0)` while the baked-hex twin stayed at the old palette — which is the whole point. The contract test caught a real defect on the way in: a track whose only animated property was a theme token was discarded by `normTrack`'s emptiness guard before the compiler ever saw it |
| **C6** | **SHIPPED, with one number honestly missing** (this commit) | **Parity is now arithmetic, not trust**: the CSS text and the WAAPI IR are compared frame by frame and property by property across every property family (transform, filter, clip, colour, SVG draw), at 0/50/100 % and on the final frame — the one a visitor with reduced motion actually sees. The only legitimate difference, the RTL mirror, is proved *by substitution*: resolving `--wjs-ix-dir` to 1 reproduces `kf`, to −1 reproduces `kfRtl`. Plus a **byte golden** of a representative sheet (855 bytes for three interactions) that pins the shape nothing else was watching: everything inside the reduced-motion media query, shorthand before `animation-timeline`, `@supports` only where a timeline is needed, the pause token only on the loop. **Cost measured**: the fallback driver's own work is **5.8 µs per frame for 20 scrub blocks** and grows linearly (10 → 1.8 µs, 40 → 3.6 µs), and in a real browser the technique's platform half (rect read + `currentTime` write over 20 real elements) is **25.5 µs**, 0.15 % of a 16.7 ms frame, with **zero long animation frames** across a 41-step scroll and a measured **input delay of 2.1 ms** under real mouse clicks. What could NOT be measured here: the same numbers *in Firefox*. Chrome cannot be made to take the fallback — the `@supports` gate lives in the stylesheet, not in JS, so patching `CSS.supports` moves the runtime's decision but not the engine's. Recorded as the open item rather than dressed up as done |
| **C5** | **SHIPPED in two halves** (`a24bdea` + `14e243a`) | **Honest pin**: a section reserves 2/3/4 screens up front and its stage holds still with `position: sticky`. Measured live: stage pinned at top 0 through 1820px of scroll, **cumulative layout shift exactly 0** across 40 scroll steps, zero JS. Motion reads the pin through a **named view timeline** the section declares (`view()` alone freezes while pinned — the block is not moving through the viewport, which is the point); default range is the pinned span (`contain`), and the fallback path anchors progress on the section instead of on the frozen block. **Governance**: a site-level `wjs_motion` policy — `full` (byte-identical default), `calm` (perpetual loops play once and settle at their *final* frame, never paused at frame 0 where a fade-in would be stuck invisible) and `off` (not one rule, not one runtime byte: measured 2107 fewer bytes on a test page, 0 keyframes, no style tag) — plus **"where does my site move"**, an inventory that compiles every page with the real compiler and ranks them by perpetual loops, then by JS-needing units. Verified in the admin UI: switching to Tranquilo took the site from 3 loops to 0 and the table recalculated live. **Role limits were refuted out**: enforcing a per-role motion quota means vetoing a save, where a false positive silently traps an author's content — the site switch plus the inventory give the same control without a gate that can lock the door |
