...
*   **Endpoints** (all `authenticate` + `isAdmin`): `GET /api/v1/marketplace/catalog` returns the merged catalog annotated with installed/active/`updateAvailable` state + a per-source status array (5-minute in-memory cache keyed by the source set, `?refresh=1` busts it); `POST /api/v1/marketplace/install` takes a catalog `id` and installs from the exact source that entry was listed under (and, when that plugin is ALREADY installed, performs the in-place update instead of 409ing); `POST /api/v1/marketplace/update` is the explicit one-click update — deactivate → swap the code keeping `plugins/<slug>/data/` → `uninstallPluginData(dropTables:false)` so the `wjp_<slug>_*` tables survive → restore the admin's grants/egress → reactivate, rolling back to the previous version on any failure; `GET`/`PUT /api/v1/marketplace/sources` read/replace the admin source list (each URL must pass the same https/localhost check; capped at 12).
*   **Install provenance (`plugin_origins`):** because an update replays the admin's grants (**`network` + the egress allowlist included** — read from the grant map alone, *not* re-gated by the new manifest) onto the replacement code and hands it the preserved `plugins/<slug>/data/`, a catalog entry may only update the plugin **it** installed. Every catalog install records `{ source, catalogId, version }` under the plugin's slug in the server-side `plugin_origins` option (`core/plugins.ts`: `getPluginOrigin`/`setPluginOrigin`/`clearPluginOrigin`; protected from plugins/themes like `plugin_grants`, and stored in the DB rather than in the plugin dir so a package can't ship its own provenance), and `updatePluginFromZip()` refuses with `409 originMismatch` when the recorded origin is absent (manual upload, or an install predating 1.12.6) or different. `uninstallPluginData()` clears it, so a re-uploaded slug inherits nothing. The whole cycle also holds a **per-slug lock** (in-process guard + `wordjs:plugin-op:<slug>` dist-lock lease; second request → `409 busy`), and `recoverInterruptedPluginUpdates()` reclaims a stash left by a killed process at boot.
