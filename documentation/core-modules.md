...
### Logic
*   `generateInstallToken()` mints a random token at boot (`crypto.randomBytes(24)` hex), prints it to the console as a ready-to-open `<siteUrl>/install#token=<token>` link, and mirrors it to a **0600 file** in the data dir (`TOKEN_FILE`) for headless/Docker installs. Idempotent for the life of the process.
