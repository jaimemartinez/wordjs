...
*   **Auth:** a shared `revalidateSecret` in `wordjs-config.json`, generated on first use through `saveConfig()` and sent as the `x-revalidate-secret` header. The endpoint can only **invalidate**, never inject — the blast radius of a leaked secret is extra renders, not integrity loss. An HTTPS frontend is verified against `certs/cluster-ca.crt` with a `['frontend','gateway']` CN allowlist (no `rejectUnauthorized: false` anywhere).
