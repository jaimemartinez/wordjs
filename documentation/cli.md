...
> **CI gate:** continuous integration (Node 22) runs the strict typecheck + `lint` + `build` + the seven phase verifiers (`verify:f0` … `verify:f6`) + the unit test suite + `perf:f0` (the F0 content performance budgets) + the **integration suite** (`npm run test:integration`, against real `postgres:16` + `redis:7` service containers), plus a **license gate** (`license-checker --production --failOn 'AGPL;SSPL'`) and a marketplace catalog integrity check (`build-marketplace.js` then `verify-marketplace.js --rebuild`). The project and all packages are MIT-licensed; dual-licensed dependencies are listed in `THIRD-PARTY-NOTICES.md`.
