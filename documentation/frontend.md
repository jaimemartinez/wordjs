...
### Archive routes (category / tag / author / date / taxonomy)

WordPress-compatible listings for a blog migrated with the WXR importer, all server-rendered, all
paginated, all fed to the theme's archive template chain.

| URL | Route file | Data source | Template chain |
| --- | --- | --- | --- |
| `/category/{slug}[/page/{n}]` | `category/[slug]/[[...paged]]/page.tsx` | `GET /categories` → term, `GET /posts?categories={id}` | `category-{slug}` → `category` → `archive` → `page` |
| `/tag/{slug}[/page/{n}]` | `tag/[slug]/[[...paged]]/page.tsx` | `GET /tags` → term, `GET /posts?tags={id}` | `tag-{slug}` → `tag` → `archive` → `page` |
| `/author/{nicename\|id}[/page/{n}]` | `author/[slug]/[[...paged]]/page.tsx` | `GET /posts?author={nicename\|id}` | `author-{slug}` → `author` → `archive` → `page` |
| `/archive/{yyyy}[/{mm}][/page/{n}]` | `archive/[...segments]/page.tsx` | `GET /posts` → narrowed on the `YYYY-MM` prefix of `post.date` | `date` → `archive` → `page` |
| `/taxonomy/{taxonomy}/{term}[/page/{n}]` | `taxonomy/[taxonomy]/[term]/[[...paged]]/page.tsx` | `GET /taxonomies/{name}`, then the same data as the dedicated URL | the dedicated taxonomy's chain |

*   **The narrowing is the backend's.** `GET /posts` filters for real on `?categories=`, `?tags=` and `?author=` — each an id or a slug, pushed into **both** `Post.findAllWithRelations` and the matching `Post.count` as a parameterised subquery (`backend/src/routes/posts.ts`) — so `src/lib/public/archives.ts` asks for the archive's posts (`getPostsBy()`) instead of reading the whole site and sorting it out in memory. It sends the term's **id** whenever it has one, since an id cannot be widened by a comma (the list grammar's OR separator) nor shadowed by a case-insensitive collation. Each archive is therefore bounded by its own size rather than by the site's, and every row fetched is a row it shows. **The date archive is the one exception, and not by preference:** the API has no date filter, so `/archive/{yyyy}[/{mm}]` still walks the published set (`getAllPublishedPosts()`, at the API's 100-row maximum) and narrows on the `YYYY-MM` prefix of the date **string**. That walk stops at `MAX_POST_PAGES` × 100 = 2 000 posts, so on a bigger site a year older than the newest 2 000 answers `404`; closing it needs a backend `?year=`/`?month=` (or `?after=`/`?before=`).
*   **Caching is the public index's, unchanged.** Every read goes through `serverFetch` with an ISR window and the `posts` purge tag, so a warm archive costs **zero** backend calls and a publish purges it instantly (`backend/src/core/frontend-purge.ts` enqueues `posts` for every content change). `/category` and `/tag` additionally `generateStaticParams` their known terms — like `[slug]/page.tsx` does for posts — which puts the landing pages in the Full-Route Cache; page 2+, authors, dates and taxonomies render on demand and are ISR-cached from there.
*   **Pagination is a path, never a query string.** Reading `searchParams` would drop the route out of the Full-Route Cache on every page and turn the archive into an uncached SQL hot path. Page size is `posts_per_page` (Settings → Reading), clamped 1…100. `page/1`, a page past the end, an unknown term, a malformed tail and an empty month are all real **404**s.
*   **Each page canonicalises to itself** (`/category/news/page/2`), stays indexable, and emits `<link rel="alternate" type="application/rss+xml" href="/{category|tag|author}/{slug}/feed.xml">` for feed autodiscovery — the public URL the scoped RSS channel is served at and prints as its own `self` link, the same root-level spelling the layout uses for the site-wide feeds. `/taxonomy/{tax}/{term}` is the exception to the canonical rule: it is an alias, so it canonicalises to the dedicated `/category|/tag` URL instead of to itself.
*   **`/archive/` rather than `/{yyyy}`, and the reserved slugs that come with it.** `(public)/[slug]` owns every one-segment public path and Next refuses two differently-named dynamic segments in the same position, so a date archive cannot live at `/2026`. The literal segments `category`, `tag`, `author`, `archive` and `taxonomy` are reserved for these routes the same way `search`, `pages` and `preview` already are — a post or page with one of those slugs is shadowed.
*   **Known gaps** (API gaps, documented in the route files at the point they bite):
    *   `/taxonomy/{tax}/{term}` 404s for a **custom** taxonomy, because terms are readable only through the two hard-coded `/categories` and `/tags` endpoints and `Post.toJSON()` serialises only `category` and `post_tag`.
...
*   **The backend generates the feeds and the sitemap; the frontend publishes them at the site's own URLs.** They are generated by `backend/src/routes/seo.ts` under the `/api/v1/seo` mount, out of `backend/src/core/seo-helper.ts` (the original single `<urlset>` and the RSS 2.0 channel) and `backend/src/core/feeds.ts` (everything added on top) — and they are *served* at the root by ten thin Next route handlers under `src/app/(public)/`: `robots.txt/`, `sitemap.xml/`, `sitemap/[chunk]/`, `feed.xml/`, `feed.atom/`, `feed.json/`, `comments/feed.xml/` and `{category,tag,author}/[slug]/feed.xml/`, each one call to `(public)/_seo/upstream.ts`. That proxy streams the upstream document through byte-for-byte and copies its `Content-Type`, `Cache-Control`, `ETag` and `Last-Modified`; an upstream `404` stays a `404` and an unreachable backend is a `502`, never a cached empty feed. The read sits in Next's Data Cache for exactly the window the upstream advertises (900s for a feed, 3600s for the sitemap, 86400s for `robots.txt`) under the `posts` tag (`settings` for `robots.txt`, whose body comes from the `siteurl` option), so a publish purge refreshes them along with the archives. **Why the layout no longer links the backend prefix:** it answers, but `robots.txt` tells crawlers `Disallow: /api/`, and no feed reader guesses a URL of that shape. The public layout (`src/app/(public)/layout.tsx`) therefore emits its three `<link rel="alternate">` tags — RSS (`/feed.xml`), Atom (`/feed.atom`) and JSON Feed (`/feed.json`) — at the public URLs, which React hoists into `<head>` on every public page. **Formats:** RSS 2.0, Atom 1.0 and JSON Feed 1.1 all render from one item source, so an entry's title, link and summary are identical across the three, and all three inherit the RSS escaping posture — a feed publishes the author's excerpt or the tag-stripped, truncated content, never raw post HTML. Scoped RSS channels exist at `/{category,tag,author}/<slug>/feed.xml` (the same channel filtered, `404` for an unknown slug; the static `feed.xml` segment outranks the archive's `[[...paged]]` catch-all, which would otherwise swallow the URL) and `/comments/feed.xml` carries approved comments on published posts, with the commenter's e-mail and IP never selected. **Every URL these documents PRINT is a public one too:** the `<loc>` of each sitemap child, each feed's `self` link and the `Sitemap:` line of `robots.txt` all come from a single `publicSeoUrl(siteUrl, doc)` in `backend/src/core/feeds.ts`, which addresses the document at the site root instead of at `siteUrl + req.baseUrl` — the mount the same `robots.txt` disallows. The archive pages advertise the same public spelling (`taxonomyFeedPath()` in `lib/public/archives.ts`). Feed length comes from the `posts_per_rss` option when an admin sets it (capped at 100) and otherwise stays at the 20 items `/feed.xml` has always sent — `posts_per_page` is deliberately *not* the fallback, since reading it would silently halve every existing site's feed. **Sitemap:** `/sitemap.xml` — the URL `robots.txt` has always advertised — stays the single `<urlset>` it has always been while the site has at most **1 000** URLs (the homepage included; `SITEMAP_MAX_URLS` in `core/feeds.ts`), and becomes a `<sitemapindex>` above that, pointing at children of at most 1 000 URLs each — `sitemap-posts-1.xml`, `sitemap-pages-1.xml`, `sitemap-terms-1.xml`, …, published at `/sitemap/<kind>-<n>.xml` (the flat `/sitemap-posts-1.xml` is not expressible in the App Router: a folder named `sitemap-[chunk].xml` is not a dynamic segment at all — Next's own `isDynamicRoute()` answers `false` for it — and a bare `/[chunk]` at the root would collide with `(public)/[slug]`). The homepage always leads the `pages` chunk, taxonomy archives (`/category/<slug>`, `/tag/<slug>`) are submitted only in index mode (below the threshold the single file stays byte-identical to what it has always emitted), each child advertises the newest `<lastmod>` it contains, and a chunk the index does not advertise is a `404` rather than a second copy of the same URLs. `robots.txt` advertises `<siteUrl>/sitemap.xml` and is itself served at `/robots.txt`, so a crawler reaches the pair from the site root without ever touching the mount. The public child URL is `<siteUrl>/sitemap/<kind>-<n>.xml`: `publicSeoUrl()` strips the `sitemap-` prefix off the backend's file name and `sitemap/[chunk]/route.ts` puts it back before proxying, and `backend/src/tests/seo.test.ts` fetches every `<loc>` the index prints through that same rebuild, so the two halves of the contract are asserted against each other rather than each against itself.
...
> **The Image block emits `<picture>` when — and only when — the attachment has modern formats.** Choosing a `src` from the media library persists what the backend reported for it (`src/lib/imageSrcset.ts`) into the block's own props: `srcSet` (the original-format width ladder), `imgWidth`/`imgHeight`, and `srcSetModern` — one srcset per modern MIME type, built from the attachment's `sources` metadata. Two places write them, and both write them in the SAME edit that writes `src` — one command, so a single undo can never leave a srcset attached to a different image: the Image block's media field (`src/lib/verso/coreBlocks.tsx`, the registry both admin editors mount) and «Recursos» in the editor toolbar, which inserts a ready-made Image block. Changing `src` to anything the picker did not report — an external URL, an address typed by hand — clears all four, so a srcset never outlives the image it describes. `ImageBlock` then renders `<source type="image/avif">` before `<source type="image/webp">` (source order *is* selection order, so the more efficient format must come first), each carrying the same `sizes` as the `<img>`, with the **unchanged `<img>` as the last child**: same `src`, `alt`, `loading`, `sizes`, intrinsic `width`/`height`, class and style. The `<picture>` wrapper is `display: contents`, so it adds no box and the `<img>` lays out against the same parent it did before. A URL is **never** derived by rewriting an extension — every candidate comes from the metadata, because the naming is the backend's business and a guessed URL is a 404. Content saved before this feature carries no `srcSetModern` and renders the bare `<img>` byte-for-byte as before; there is no server-side backfill for it (the API offers no lookup from an upload URL back to its attachment), so such a block gains its variants the next time an author picks the image again. The same bare `<img>` is what any attachment whose upload produced no derivatives renders — an animated GIF, an SVG, a host whose sharp cannot encode, or a photo over the encode budget, where the pipeline gives up the whole format rather than writing a ladder that would cap every modern browser below the original's width.
...
`next.config.ts` `headers()` sets a baseline Content-Security-Policy (and companion headers) on **every** route (`source: '/:path*'`). What follows is the **production** shape — `script-src` is the one environment-dependent directive, and every `NODE_ENV` other than `development` (including `test`) gets this one, so it is what `next start` serves and what the tests pin:
...
Companion headers: `X-Frame-Options: SAMEORIGIN`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy: camera=(), microphone=(), geolocation=(), browsing-topics=()` (`payment` is simply not listed, so the Payment Request API keeps its browser default — note the old justification, "the online-store plugin's Stripe Elements uses the Payment Request API", was **wrong**: that plugin creates a Checkout Session server-side and the browser leaves by top-level navigation to Stripe's hosted page, which this header does not govern). `poweredByHeader: false` also drops Next's `X-Powered-By` version fingerprint. A second rule caches `/fonts/:path*` with `public, max-age=31536000, immutable`.
...
*   **`script-src` is permissive by necessity, so it is not the XSS control** — the server-side sanitizer in `sanitize.ts` is. Its directives stay because removing them is a regression:
    ...
    *   `'unsafe-eval'` — **removed from the production header.** The retired Puck editor was its reason. The real production client build contains zero `eval(` and zero `new Function(`; the only two `Function("` occurrences are core-js's and decimal.js's global-object fallbacks, which short-circuit on `globalThis`/`self` and never execute in a browser. No WebAssembly in the client chunks, and the shipped plugin admin bundles are clean too. A **development** build (`NODE_ENV === 'development'`) still carries the keyword, because React's development build calls `eval()` to reconstruct component stacks and logs "React requires eval() in development mode" on every page under the strict header; React never uses it in production. `src/lib/__tests__/embedHostsCsp.test.ts` runs under `NODE_ENV=test` — the strict branch — reads the real header and fails if the keyword comes back, including if it hides in `script-src-elem`, `script-src-attr`, `worker-src` or `default-src`.
*   **`https:` on `font-src`/`style-src`/`img-src`/`script-src`** covers the app's own theme assets (fonts under `/uploads/fonts`, theme CSS, images). The old second justification — the retired editor rendering the theme in an `about:srcdoc` iframe, where `'self'` does not resolve to the page origin — is **dead**: Verso's canvas is an ordinary same-origin route (`<iframe src="/admin/canvas-frame">`) and no `srcdoc` iframe remains in the frontend. What keeps `https:` in **`script-src`** today is the bundled `analytics-tag` plugin: its public loader (`marketplace/plugins/analytics-tag/public/loader.js`) injects `<script>` elements for Google Tag Manager, Plausible, and an **admin-entered Matomo origin** (validated only as `https`). This header is baked at build time and cannot know a per-site origin, so no explicit allow-list can replace `https:` yet. Narrowing it means proxying Matomo same-origin first, then listing explicit origins; removing it outright disables third-party analytics tags and is a product decision.
