...
*   **Driven by `--wjs-*` design tokens** declared in each theme's `style.css :root` (not `theme.json`). The framework has fallbacks, so a theme re-skins everything just by setting tokens. The default theme ships bundled, and 12 first-party themes are available via the theme marketplace — each ships a full `--wjs-*` token set tuned to its palette (including per-variant `--wjs-color-on-*` max-contrast text tokens).
...
*   **API client:** `marketplaceApi` in `src/lib/api.ts` — `catalog(refresh?)` → `GET /marketplace/catalog`, `install(id)` → `POST /marketplace/install`, `update(id)` → `POST /marketplace/update` (one-click in-place update of an installed plugin: data, granted permissions and active state are preserved, and a failed package rolls back; refused with `409 originMismatch` when the plugin was not installed from that catalog source — including manually uploaded plugins, which have no recorded origin), `getSources()` → `GET /marketplace/sources`, `setSources(urls)` → `PUT /marketplace/sources` (admin-configurable catalog sources — no hard-coded URL).
...
*   **Install flow:** installs run through the same hardened pipeline as plugins — the backend downloads the zip server-side, verifies its **sha256** against the catalog entry, and runs it through the zip-guard before `installThemeFromZip()`. The 12 first-party themes ship in the default catalog.
