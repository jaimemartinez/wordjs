...
`npm test` runs the backend unit suite across 66 `src/tests/*.test.ts` files (node:test + supertest). Backend `lint`/`format` are **local commands** — backend ESLint is **not** a CI gate.
