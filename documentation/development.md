...
```bash
cd backend
npm test               # node --test --test-force-exit --test-concurrency=1 -r ts-node/register src/tests/*.test.ts (node:test + supertest)
npm run test:integration  # node --test … src/tests-integration/*.test.ts (needs Postgres + Redis)
npm run lint           # eslint (flat config)
npm run format
```

`npm test` runs the backend unit suite across 207 `src/tests/*.test.ts` files (node:test + supertest). `npm run lint` **is** a CI gate (the backend job runs it between the typecheck and the build; ESLint exits non-zero on errors only, so warnings are surfaced but tolerated). `npm run format` stays a local command.
...
`.github/workflows/ci.yml` runs seven parallel jobs on every push/PR (Node 22):
