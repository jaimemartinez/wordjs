...
*   **The 3-Strike Rule:** To avoid "false positives" (like a power outage during plugin load), CrashGuard uses a strike system.
    ...
    2.  **Strike 3:** If the plugin consistently crashes the server 3 times (`MAX_STRIKES = 3`, `backend/src/core/crash-guard.ts`), it is **automatically disabled** — dropped from the active list under a dist-lock — and a persistent `error` entry is appended to the `admin_notices` option (`backend/src/core/plugins.ts`). That append is **best-effort**: a lock or option failure is logged and swallowed, because wedging crash recovery is worse than losing a notice. The notice is readable at the admin-only `GET /api/v1/settings/notices`, but **no admin-panel screen renders it today** (nothing in `frontend/src` reads that endpoint), so the dependable operator signal is the server log.
...
*   **Syscall surface / kernel hardening (default-on, opt-out):** A **default-on** Linux layer (`config.sandbox.useKernelHardening`, opt out with `=false`; via `bubblewrap`, probe-validated per host — falls back to plain fork isolation on a host lacking `bwrap` / unprivileged user-namespaces; a no-op on Windows/macOS) launches each isolated child as an **unprivileged uid (nobody) with all Linux capabilities dropped, `no-new-privs`, PID/IPC/UTS namespaces, and a read-only filesystem** (the app root stays writable so plugin storage keeps working; network is preserved and still egress-guarded; the resident RSS poll sums the bwrap subtree so the memory cap keeps biting). It composes with the cgroup/rlimit memory cap and is fail-safe — any probe failure falls back to the standard isolated launch (zero regression). By default a probe failure degrades to the JS-guards-only launch, but you can set `config.sandbox.requireHardening=true` to **fail closed** — a plugin is then refused unless the bwrap/seccomp backstop is actually ACTIVE. The live hardening state (`active` / `degraded` / `disabled` / `unsupported`) is reported on admin `GET /health/details`, alongside the two independent layers below (`netns` and `permission`, `backend/src/core/system-health.ts`). **Kernel network namespace (`config.sandbox.unshareNetwork`, default-on, opt-out, Linux + bwrap only, separately probe-gated):** a plugin **without** the `network` grant is additionally dropped into its **own empty network namespace** (`bwrap --unshare-net`), so it cannot reach cloud metadata, host loopback or the internet **at the kernel level** — not merely through the in-process JS neuter. A `network`-**granted** plugin keeps the shared namespace (its sockets must work) and remains bounded by the connect-time egress guard alone. **Node permission model (`config.sandbox.usePermissionModel`, default-on, opt-out, EVERY platform — the only OS-level confinement Windows and macOS get):** the child is launched under Node's own C++-enforced permission model with `--allow-fs-read` scoped to the app root and `--allow-fs-write` scoped to exactly the zones io-guard permits, while `child_process` / `worker_threads` / native addons / WASI are simply never granted — denied below JavaScript rather than by a JS proxy. Probe-gated (the flag name moved between Node versions and a build can accept it without enforcing it, so a child must actually be refused a read before it activates) and, like `blockCodeGen`, not applied under `ts-node`. Node's permission model has **no** `--allow-net` token, so the JS egress guard is — and always was — the sole authority on where a plugin's traffic may go. It also applies a **`seccomp`-bpf syscall DENYLIST** (assembled in pure JS, no native dep, via `bwrap --seccomp`): `ptrace`, `mount`/`umount2`/`pivot_root`, `kexec_*`, `*_module`, `bpf`, `keyctl`/`add_key`/`request_key`, `userfaultfd`, `setns`, `process_vm_readv`/`writev`, `perf_event_open`, `reboot`, `open_by_handle_at`, … → `EPERM` (syscalls a Node app/web plugin never issues but that are escape/kernel-manipulation primitives). The probe boots Node UNDER the filter to prove it doesn't break plugins. Validate it on a host with `node backend/scripts/verify-sandbox-hardening.js`. The **`Landlock` LSM** is intentionally **not** used — its filesystem-confinement goal is already met by the read-only mount namespace above, and the LSM itself would need a native dependency contrary to this sandbox's no-native-deps design. **Trade-off:** the uid-drop means a plugin **cannot bind a privileged port (`<1024`)** under hardening (e.g. the mail-server, whose inbound listener defaults to port **25** — it probes before binding and falls back to the unprivileged **2525** with a *degraded* inbound status rather than losing mail). A *preventive* memory cap on Windows now **ships** as a Job Object (`ProcessMemoryLimit`, default-on, probe-gated, pure-JS PowerShell P/Invoke — see §4); the reactive `tasklist` RSS poll remains as a backstop for the brief post-fork assign window.
...
| Outbound network | **blocked** unless the `network` capability is granted (admin opt-in, exfiltration warning) — and on Linux under kernel hardening an ungranted plugin is also placed in an empty network namespace (§4). |
