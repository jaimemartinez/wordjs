...
> **Native kernel sandbox — default-on, fail-closed, on every production platform.** Underneath the
> OS-process boundary above, every production plugin is additionally wrapped by the kernel mechanism its
> host provides: **Landlock + seccomp-bpf** on Linux, **AppContainer** on Windows, **Seatbelt** on macOS.
> Each path is activated only after a real confined child **and an unconfined control** prove scoped
> filesystem access, process/dangerous-syscall denial, working storage + IPC, and **both** network-policy
> shapes — a platform name or a successful API call is not evidence. A `network` grant changes only
> egress; it never removes the native sandbox. `config.sandbox.requireHardening` defaults to **true**, so
> an unavailable probe or a failed per-plugin native launch **refuses the plugin** (setting it to `false`
> explicitly permits the weaker compatibility fallback). Linux needs no namespace, sysctl, daemon, root
> privilege or host tuning: `/usr/bin/perl` applies Landlock, `no_new_privs` and the seccomp filter to
> itself and then `exec`s Node — Landlock scopes readable and writable paths, prevents cross-domain
> process access, and on ABI 6+ scopes abstract Unix sockets and signals, while seccomp rejects
> kernel-control, cross-process-memory, SysV IPC, signal, namespace/mount/modern-IO, anonymous-executable
> (`memfd_create`/`execveat`) and process-creation paths (`clone` is retained only with `CLONE_THREAD` so
> Node/libuv threads work, and on x86_64 the x32 ABI is rejected). Without the network grant every new
> socket is rejected; with it, only AF_INET/AF_INET6 client sockets are admitted (with independent
> Landlock TCP rules on ABI 4+). AppContainer restricts access to objects ACL'd for its package SID and
> its Job Object admits exactly the contained Node process while rejecting descendants, adding only
> `internetClient` for a granted plugin; Seatbelt allowlists the required runtime/app reads and writable
> zones, denies process inspection, and changes only its outbound-network allow — `process-fork` and
> `process-exec` are deliberately **allowed**, because a descendant inherits the same Seatbelt domain, so
> the process tree is contained rather than forbidden. Neither OS pretends to have Linux namespaces or a
> Unix uid drop — the guarantee is **equivalent authority reduction**, not identical primitives. The
> **Node permission model** (every compiled platform, default-on, probe-gated) sits under all of it as an
...
*   **Shipped dependencies are scanned too:** the scanner reads the plugin's own source **in full**, and also walks the `node_modules/` a plugin **ships** (present only for an *uploaded* plugin — a marketplace plugin declares its dependencies and the host installs them). Dependency files are run through the **same detector** as the plugin's own code — no second copy of the rules — and each hit is reported prefixed with its path (`node_modules/<pkg>/…: require('child_process')`). Two calibrations differ on purpose inside a dependency: the gates that are pure noise in third-party code are dropped (computed `require()`/dispatch, `process.platform`-class reads, bare `exec`/`spawn` *names*, `global`/`module` reads), while the sinks that have no benign reading are kept (`require`/`import` of a sensitive builtin, `eval`/`new Function`/indirect codegen, and the native-binding escapes `process.binding`/`_linkedBinding`/`dlopen`/`getBuiltinModule`). A dependency's `fs` use is left to the runtime `io-guard` (which charges it to the plugin's grants at the call) instead of being charged statically, and its **networking builtins** (`secure-require`'s own `NETWORK_MODULES` list — `http`/`https`/`net`/`dns`/… — so there is one population, not two) are charged as a **`network` permission** rather than blocked: the runtime already gates exactly those on the admin's Network grant for anything under `plugins/<slug>/`, so hard-blocking them would make the install-time scan stricter than the sandbox and refuse a network-granted plugin that ships `express` or `nodemailer` for a call the sandbox would have allowed. Everything the runtime does *not* unlock with a grant (`child_process`, `vm`, `worker_threads`, …) stays a hard block. **Hitting a bound is itself a finding:** a tree too large, too deep, holding a file too big to parse, or containing a symlink that escapes the plugin directory fails validation — an unscannable dependency tree is not a pass. The bounds themselves, and what this pass deliberately does **not** claim, are in §4.
...
> **Detached Code is Still Sandboxed:** Plugin code that runs *outside* the `AsyncLocalStorage` wrapper — Express route handlers a plugin registers, synchronous hooks, timers (`setTimeout`/`setInterval`), and module top-level code — used to run with an empty context and was therefore treated as trusted core (a real RCE bypass). The runtime guards now resolve the active plugin via `getEffectivePlugin()`, which uses the `AsyncLocalStorage` context **or**, when absent, the nearest plugin/theme frame on the call stack. Synchronous hooks (`doActionSync`/`applyFiltersSync`) also run their callbacks inside the plugin context. The **microtask family** (`setImmediate`/`queueMicrotask`/`process.nextTick`) and **EventEmitter listener registration** (`on`/`once`/`addListener`/`prepend[Once]Listener`) are anchored the same way — captured at schedule/registration time, re-entered when the callback fires — with the cheaper ALS-only `getCurrentPlugin()` check rather than the stack walk, so core hot-path schedules with no plugin context are untouched. (An earlier code comment claimed the microtask family was intentionally left unwrapped; it is not, since a deferred `require('child_process')` in a never-scanned `dist/` file was found. Listener registration needs its own anchor because `emit()` does **not** propagate the `AsyncLocalStorage` context — the seed of the detached-listener RCE.) As a result, detached plugin code remains subject to its manifest permissions.
...
WordJS provides a high level of isolation, but it is not a virtual machine, and it has **not** had an independent security audit. The controls these residuals sit on top of are documented in §1 (sandbox), §1.1 (static analysis), §1.2 (runtime guards), §9 (CSRF) and §10 (security headers); what follows is only what is still **open**.
*   **CSP is not an XSS backstop yet (§10).** The production `script-src` still carries `'unsafe-inline'`, for the Next.js App Router's inline bootstrap/hydration scripts — a per-request **nonce** migration is the documented follow-up; `https:`, **solely** for the bundled `analytics-tag` plugin's third-party tags, which cannot be replaced by a static allow-list while one of them is an **admin-entered Matomo origin** a build-time header cannot know; and `blob:`, without which every plugin admin bundle and its icons fail to render. Until the nonce lands, the header's value is **structural** (`frame-ancestors 'self'`, `object-src 'none'`, `base-uri 'self'`) and the XSS control is the server-side sanitizer in `lib/sanitize.ts` (§7). (`'unsafe-eval'` is **not** a residual — it is gone from every production surface; see §10.)
*   **The gateway and backend CSPs are permissive mirrors (§10).** Both carry an open `img-src`/`connect-src` (`*`); the structural directives are present (`frame-ancestors 'self'`, `base-uri 'self'`, `form-action 'self'`, `script-src-attr 'none'`, `object-src 'none'` — Helmet's defaults fill in what the config does not name, as the emitted header shows), and each governs only the responses that origin generates itself — a proxied response keeps the upstream's own policy, because the upstream writes its headers last.
*   **CSRF token scope (§9.1).** The double-submit token is a per-browser random value, not one derived from the session secret, and it rotates only when a session cookie is issued — not per request. And the gate keys on *a verifiable session cookie*, so a request that carries none — or one that no longer verifies — is not gated: correct (there is no ambient authority to abuse either way), but it means the token is not an anti-automation or anti-replay control.
*   **The shipped-dependency scan is bounded (§1.1).** A plugin's own source is read in full, but its shipped `node_modules/` is read only up to **4000** files, **`.js`/`.cjs`/`.mjs`** only, nothing larger than **1 MB** (which skips minified bundles), a **32**-level depth cap, and **no symlink followed outside the plugin directory**. A tree past any of those bounds is **refused rather than scanned** — reported as a finding, never waved through — and inside a dependency several name-based detectors are deliberately relaxed. So this is a review aid, not a supply-chain audit: it says "nothing obvious was found in what we could read", and **runtime containment** (`secure-require`, `io-guard`, the worker's ESM hook, `blockCodeGen`) remains what actually *stops* a dependency.
*   **Resource limits are not preventive everywhere (§1.2).** macOS has **no unprivileged preventive resident-memory primitive** — its `RLIMIT_AS` spelling is probed and rejected as unenforced — leaving the V8 heap ceiling, process separation and the reactive RSS poll; that memory-parity gap is unchanged and is not described as solved. The preventive CPU quota (`config.sandbox.cpuQuotaPercent` — one knob, spelled as a Windows Job Object rate cap or as `CPUQuota` inside the systemd `--user` scope that `useCgroupMemoryCap` creates) is **opt-in**, default `0` = off, so on Windows without it, on a plain Linux launch, on macOS, and on any host whose cgroup probe failed the only CPU bound is the **reactive** watchdog: it bounds a burn rather than preventing it, and an attacker who idles one tick per minute is out of scope by design. **Linux cgroup mode enabled without that quota is the one configuration with no CPU bound at all** — the child the host can see is `systemd-run`, not the node process, so the watchdog cannot sample it; the server logs a warning at launch naming the knob. On macOS it also demotes itself — a host whose `ps` rejects `-o cputime=` falls back to the rss-only query and logs that the CPU watchdog is **off** for that child (the resident cap is unaffected).
*   **Host requirements for the native sandbox (§1).** Linux needs Landlock (kernel 5.13+) and `/usr/bin/perl`; a host missing either cannot apply the confinement at all, and because `requireHardening` defaults to true the plugin is **refused** rather than started with only the Node permission model and the JavaScript guards (which is what `requireHardening: false` opts into). The one exemption is the Windows **source-only `ts-node` development** worker, where first-run recursive ACL provisioning over the TypeScript dependency tree exceeds development test deadlines; compiled production has a bounded runtime tree and no exemption.
...
*   **Admin-enforced MFA-by-role policy:** an administrator can *require* MFA for chosen roles, with a **grace window in days**, via `GET`/`PUT /auth/mfa/policy` (`authenticate` + `sessionOnly` + `isAdmin` — an API token, even an admin's, can never reconfigure it). A subject user who has not enrolled is only **nudged** while inside the window (login and `/auth/me` carry `withinGrace` + `graceDeadline`); once it lapses, the global `mfaComplianceGate` (`backend/src/middleware/auth.ts`) answers `403 mfa_enrollment_required` to everything except the enrolment/session allowlist (`/auth/me`, `/auth/logout`, `/auth/refresh`, `/auth/mfa/{setup,enable,status,backup-codes,disable}`) — deliberately **not** `/auth/mfa/policy` or `/auth/tokens`, so an enforced admin cannot disable the requirement or mint a headless token instead of enrolling. The grace anchor is `max(policy.enforcedAt, the user's registration)`, and `enforcedAt` is stamped server-side on the transition to ≥1 required role, so it cannot be back-dated to expire everyone's grace at once. The gate fails **open** if the policy row itself is unreadable (a site-wide option blip must not lock everyone out) and **closed** (`503`) if one user's compliance can't be evaluated. `wjt_` API tokens are exempt **by default** (`enforceForApiTokens: false` — an existing install behaves exactly as before), because a machine client cannot answer a TOTP challenge. That default used to be the whole story, and it left a documented residual: a token minted *before* the policy took effect kept working, so turning on "MFA required for administrators" left every pre-existing admin token as an MFA-less door and revoking each by hand was the only remedy. The policy now carries an opt-in — **Also enforce for personal API tokens** in *Settings → Security*, `enforceForApiTokens: true` on `PUT /auth/mfa/policy` (validated as a boolean; omitted means off, like the other two fields). With it on, a `Bearer wjt_…` request whose owner is in a required role and has not enrolled past grace is refused with the **same** `403 mfa_enrollment_required` shape the cookie path uses, differing only in the hint: the fix is for the token's **owner** to enrol (or for an administrator to revoke the token), since the request itself can never complete a second factor. What it does **not** do: demand TOTP over Bearer. A token whose owner *has* 2FA passes untouched — it was minted from an MFA-protected session, and `POST /auth/tokens` already refuses a policy-subject un-enrolled user — the enrolment allowlist is unchanged, and an unrecognised `wjt_` string is still answered by the route's own `401` rather than by the gate, so this never becomes an oracle for which tokens exist.
...
*   **CSRF:** the `Bearer wjt_…` path is the sanctioned non-browser API-caller path referenced by the fail-closed CSRF rule in §9 — it carries no ambient cookie, so it is exempt from **both** halves of the check: the Origin/Referer rule and the `X-CSRF-Token` double-submit gate (§9.1).

### Audit log

Security-relevant mutations are recorded in the append-only `audit_log` (`backend/src/core/audit.ts`, migration `0009`), read back through the admin-only `GET /api/v1/audit`; no route updates or deletes a row. Recorded today: **authentication** — `auth.login.success` / `auth.login.failure` (the attempted username and the client IP, **never** the password; only the credential check is logged, so a brute-force run's write rate is bounded by the lockout ladder rather than by the attacker), `auth.logout`, `auth.password.reset`, `auth.mfa.enable` / `.disable` / `.backup_codes`, `auth.token.create` / `.revoke`; **content** — `post.create`, `post.update`, `post.publish`, `post.trash`, `post.restore`, `post.delete` (one row per request, naming the most specific transition, with `from`/`to` status — editor autosaves are excluded, exactly as they are excluded from the revision history); **operations** — `backup.create` / `.restore` / `.delete`, `marketplace.install` / `.update` (source, version and whether the package carried an integrity hash; failures too), `plugin.activate` / `.deactivate`, `theme.activate`, `settings.update`, and the `user.*` events (create, delete, role change, session revocation, `user.mfa_reset`). Every action name lives in one catalogue (`AUDIT_ACTIONS`) that a test derives from the source, so a new call site cannot invent a spelling. Details are sanitized before storage — secret-named keys and nested objects are dropped — and no content bodies are ever copied in. **Retention:** a daily cron prunes rows older than the `audit_retention_days` option (default **365**; `0` keeps everything for ever), logging how many it removed and recording an `audit.prune` row so a thinned log is distinguishable from a quiet one. Two call sites are still missing and tracked: the self-service password change (`routes/users.ts`) and the media routes.
...
6. **Install token (pre-install takeover)**: Before the instance is installed, the `/setup/install` and `/setup/test-db` endpoints are gated by a **one-time install token** (`backend/src/core/install-token.ts`). It is generated at boot when the instance is not yet installed and **written to a `0600` file** in the data dir (`data/install-token`), which is the reliable source for headless/Docker reads. It is **printed to the console only when stdout is a TTY** — an operator is genuinely watching a terminal — or when `WORDJS_PRINT_INSTALL_TOKEN=1` says the operator has decided their log sink is trustworthy (`shouldPrintBootstrapSecret`, which also gates the generated initial administrator password in `index.ts`). Off a TTY the boot banner prints the `/install` URL **without** the `#token=` fragment and names the file instead: the backend's console output is bridged into structured JSON that operators are told to ship to a log aggregator, where a printed bootstrap secret becomes a durable, indexed, searchable credential. When it *is* printed it is a ready-to-open `…/install#token=<token>` link — the token rides in the **URL fragment**, which the browser never transmits, so it cannot land in access/proxy logs or in a `Referer`, and the install page scrubs it from the address bar on arrival (a legacy `?token=` is still parsed, as a compatibility ramp for an older printout, not as a supported transport). An operator may supply their own via the `WORDJS_INSTALL_TOKEN` env var, but a value **under 16 chars is ignored** (with a warning) and a random token used instead — an entropy floor so a short/guessable value can't reduce the takeover gate to a brute-forceable secret. The token is checked **constant-time**, held in memory (fail-closed if not generated), and the on-disk mirror is removed once installed.
...
*   **The gate keys on a VERIFIABLE session cookie**, not on the request having been authenticated. `csrfProtection` runs globally, before any route's `authenticate`, so waiting to learn the answer would mean re-mounting the gate on every router, including ones that do not exist yet. A cookie that verifies is the fail-closed proxy; one that does not is let through, because it authenticates nothing and there is therefore no ambient authority for the token to protect. That keeps the gate's meaning honest — the token protects what the cookie *authenticates* — and it is what a client whose **first** request is a mutation depends on: an API script, a cookie jar or a stale tab that posts straight away never passes through the back-fill in the next bullet, so with a dead cookie attached it would otherwise get a `403` (instead of the honest `401`) on register, forgot-password, reset-password, verify-email, logout and the rest. The invariant is that **the gate never skips a cookie `authenticate` would accept**: it calls `verifyToken` — the very function `authenticate` calls, with no clock tolerance and no leniency of its own — and `authenticate` only ever rejects *more* on top of that (revocation, a deleted user, the MFA policy gate), never less. The two can therefore only disagree in the fail-closed direction: a revoked-but-unexpired cookie still verifies here and stays gated.
*   **Bootstrap.** A browser that holds a session cookie but no `wjs_csrf` cookie is minted one on its next **safe, authenticated** request (`ensureCsrfCookie`, called from `authenticate`/`authenticateAllowQuery`). Without that, a still-valid session issued before this feature shipped could neither mutate nor log out — both are cookie-carrying mutations, and its cookie *verifies*, so the gate holds them — and would have to be thrown away and re-established. It is called **before** the session is verified, so a browser whose session has since **expired** is back-filled on the same request (`GET /auth/me` answers `401`, but the `Set-Cookie` rides along) and is not left tokenless either; what the verify-first rule above covers is the client that never makes a safe request at all. Minting cannot be abused: the value is server-generated randomness that a cross-origin page can no more read from a `Set-Cookie` than from the cookie jar. It is deliberately **not** done from `optionalAuth`, which runs on public, cacheable routes where a shared cache could hand one visitor's token to another.

**What a third-party client must send.** Authenticating with the **session cookie**: read the `wjs_csrf` cookie (readable from JS by design) and send its value as `X-CSRF-Token` on every `POST`/`PUT`/`PATCH`/`DELETE`, alongside an `Origin` that matches the site. Authenticating with **`Authorization: Bearer wjt_…`**: send neither — the Bearer path is exempt from both halves of the CSRF check. In the browser, `frontend/src/lib/csrf.ts` is the one reader every fetch surface uses. A **cross-origin** cookie client additionally needs the preflight to pass: `X-CSRF-Token` is advertised in the backend's `Access-Control-Allow-Headers` (an unlisted custom header would fail the preflight before the gate could ever run), but only for an origin the CORS allow-list already admits.
...
> `img-src` include `*`, while `frame-ancestors 'self'`, `base-uri 'self'` and `form-action 'self'` come
> from Helmet's defaults; the frontend policy below is the one that governs the pages a user actually
> browses.) The policy is `default-src 'self'` with `frame-ancestors 'self'`, `object-src 'none'`, and
> `base-uri 'self'` — its real value is **clickjacking / structural**.
> `script-src` is `'self' 'unsafe-inline' blob: https:` in production. **`'unsafe-eval'` was removed** from
> that shape on **every** production surface that sets a policy — frontend, gateway and backend. The
> retired Puck editor was its only reason; the evidence for dropping it is the real production client
> build, which contains zero `eval(` and zero `new Function(` (the only two `Function("` occurrences are
> core-js's and decimal.js's global-object fallbacks, unreachable in a browser because they short-circuit
> on `globalThis`/`self`), no WebAssembly, and clean shipped plugin admin bundles. A
> `NODE_ENV=development` frontend build still emits it, because React's development build calls `eval()`
> to reconstruct component stacks and logs on every page without it; the gateway and backend policies are
> **static** and carry it in no environment. The **backend** dropped it too: the only HTML that origin
> serves is the Swagger UI page at `${prefix}/docs`, and `swagger-ui-express` emits **all three** of its
> scripts as **external** tags (`./swagger-ui-bundle.js`, `./swagger-ui-standalone-preset.js`, and the
> bootstrap itself as `./swagger-ui-init.js` — `swagger-ui-express/index.js`). The page's only inline
> element is the `customCss` `<style>` block, which `style-src` already covers, so `script-src`
> `'unsafe-inline'` on the backend is a **holdover, not a requirement**; narrowing it is a follow-up (the
> backend code comment still repeats the inline-bootstrap story and is tracked separately). Nothing on
> that surface builds a script from a string. Regression shape tests pin the absence on all three
> surfaces (`gateway/test/security-headers.test.js`,
> `frontend/src/lib/__tests__/embedHostsCsp.test.ts`, `backend/src/tests/public-surface-hardening.test.ts`).
> What is left, and why:
> `'unsafe-inline'` for the Next.js App Router's inline bootstrap/hydration scripts (a per-request
> **nonce** migration is the documented follow-up); `blob:` for plugin admin bundles loaded via
> `import(URL.createObjectURL(blob))` (`lib/pluginBundleLoader.ts`), without which every plugin admin UI
> and its icons fail to render; and `https:` **solely** for the bundled `analytics-tag` plugin's public
> loader (`marketplace/plugins/analytics-tag/public/loader.js`), which injects `<script>` elements for
> `https://www.googletagmanager.com/gtag/js`, `https://plausible.io/js/script.js` and an **admin-entered
> Matomo origin** validated only as `https` — the header is baked at **build** time, so it cannot know a
> per-site origin, which is exactly why a static allow-list cannot replace `https:` today. The path to
> narrowing it: proxy the admin-entered Matomo origin same-origin (or emit the header per request), then
> swap `https:` for an explicit origin allow-list; dropping it outright disables third-party analytics
> tags and is a product decision, not a cleanup. So for your threat model still do **not** treat the CSP
> as an XSS backstop — the XSS control is the server-side sanitizer (§7). §4 states the residual plainly.
