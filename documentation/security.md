...
*   **CSRF:** two checks, AND-ed (see §9): **origin matching** (exact, on Origin/Referer plus a gateway-pinned `X-Forwarded-Host`, failing closed when both headers are absent unless the caller is a `Bearer` client) **and** a **double-submit token** (the `wjs_csrf` cookie echoed in `X-CSRF-Token` on every cookie-authenticated mutation). What remains a limitation: the token is a per-browser random value, not one derived from the session secret, and it rotates only when a session cookie is issued — not per request. And the gate keys on *the session cookie being present*, so a request that carries no session cookie is not gated: correct (there is no ambient authority to abuse), but it means the token is not an anti-automation or anti-replay control.
...
*   **Who is exempt.** `Bearer` callers (a `wjt_` API token, or a session JWT sent as a header) — no browser attaches those automatically, so there is no ambient authority for a hostile page to ride; the same boundary `isHeadless` draws elsewhere. And `GET`/`HEAD`/`OPTIONS`, which never reach the gate.
...
*   **The gate keys on the session cookie being present**, not on the request having been authenticated. `csrfProtection` runs globally, before any route's `authenticate`, so waiting to learn the answer would mean re-mounting the gate on every router, including ones that do not exist yet. Cookie-present is the fail-closed proxy.
*   **Bootstrap.** A browser that holds a session cookie but no `wjs_csrf` cookie is minted one on its next **safe, authenticated** request (`ensureCsrfCookie`, called from `authenticate`/`authenticateAllowQuery`). Without that, a session issued before this feature shipped could neither mutate, nor log out, nor log in again — each of those is a cookie-carrying mutation — leaving *delete your cookies by hand* as the only escape. Minting cannot be abused: the value is server-generated randomness that a cross-origin page can no more read from a `Set-Cookie` than from the cookie jar. It is deliberately **not** done from `optionalAuth`, which runs on public, cacheable routes where a shared cache could hand one visitor's token to another.
