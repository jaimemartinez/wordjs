...
| Method   | Endpoint            | Auth  | Description                                      |
| :------- | :------------------ | :---- | :----------------------------------------------- |
| `GET`    | `/posts`            | Opt.  | List posts (filters: `type`, `status`, `search`, `author`, `categories`, `tags`; ordering: `orderby`, `order`) |
...
| `POST`   | `/comments`         | Opt.  | Create a comment (guests allowed; rate-limited, honeypot + repeat guard — see note below) |
...
> **Taxonomy and author filters — `?categories=`, `?tags=`, `?author=`.** Each takes one **comma-separated list**, and each element is either a **row ID** (all digits) or a **slug** — a term's `slug`, or an author's `user_nicename` falling back to `user_login`, which is the same identity `GET /seo/author/:slug/feed.xml` resolves and the same value `Post.toJSON()` hands back in `author.slug`. Semantics are WordPress's: **OR within one list** (`?categories=news,3` matches a post in either term) and **AND across the parameters** (`?categories=news&tags=react` requires both, and `?author=` narrows on top of them). The filters are applied in `Post.findAllWithRelations` **and** in `Post.count`, through the shared `buildWhere()`, so `X-WP-Total`/`X-WP-TotalPages` describe the filtered set and page *n* of a filtered list is reachable. They are a *narrowing*, never a widening: the status/visibility gate below still runs, so a taxonomy filter cannot surface another author's draft.
>
> `?author=<slug>` exists so a **public** author archive can be listed without an authenticated users endpoint — `/users` requires a session, so a public page had no nicename→id path at all and the archive could only be addressed by numeric id.
>
> A malformed element answers **`400 rest_invalid_param`** with the offending parameter named in `data.params`, rather than being dropped (a silently narrowed list answers a question nobody asked). Refused: an empty element (`1,,2`, `1,`), an all-digit element outside the id range enforced by `core/query-params.isRouteId` (`0`, `9999999999` — the latter is `22003 value out of range` on Postgres), an element carrying a control character or longer than 200 characters, and more than 50 elements. An **entirely empty** value (`?categories=`) means *absent*, not malformed — the same reading `?type=` gets. Being read at all also brings them under the scalar-query rule (§4), so `?categories[]=news` is a `400` like every other repeated/bracketed scalar.

> **The author on the wire.** Every serialised post carries `author: { id, displayName, slug }` — `displayName` is `users.display_name` (falling back to `user_login`), `slug` is `user_nicename` when populated and `user_login` otherwise — plus the raw `authorId` for callers that only want the number. This is the shape the generated content contract (`ContentRecord`) has declared since F2; the model used to emit the bare id, so `post.author.displayName` was `undefined` on every response and the front-end byline, OpenGraph `authors` and JSON-LD `author` all silently fell back. Nothing else from the `users` row is exposed — an author byline is not an account listing. Listings resolve every author in **one** query (`Post.hydrateRelations`), so the object costs no N+1.
...
> **Public-endpoint posture (`POST /comments`), in order.** This route is anonymous by default (`comment_registration` is `"0"` until an operator turns it on) and writes a permanent row per call, so it carries the layers `POST /forms/submit` does (limiter, hard bounds, the `_hp` honeypot under the same field name) plus a repeat guard, in the order below. **(1)** A dedicated per-IP limiter — **5 comments / 10 min** for an anonymous caller, **30 / 10 min** for an authenticated one, refused with **`429 rest_comment_rate_limited`**. Both ceilings are options an administrator can move — `comment_rate_anon` (default `5`) and `comment_rate_auth` (default `30`), per 10-minute window, clamped to `1..1000`: `0` or junk narrows the door, it never removes it. The two tiers are separate key spaces (anonymous keyed by client IP, authenticated by user id), so a logged-in author cannot spend the strangers' budget behind a shared NAT address and vice versa. Unlike its siblings the limiter is mounted on the router's own `POST` declaration rather than in `backend/src/index.ts`: an app-level mount runs before that router's `optionalAuth`, so the authenticated tier would be a branch that never applies, and a prefix mount would charge a moderator's `POST /comments/:id/approve` to the public write budget. `index.ts` still hands it the shared Redis store, so a multi-node install enforces one cap across all nodes. **(2)** A hard body bound of **64KB** (`400 rest_comment_too_long`), checked with the rest of the validation and therefore *before* the honeypot, so a bot sees byte-identical behaviour whether or not it tripped the trap. **(3)** A repeat guard, deliberately **before** the honeypot: the same body, from the same author (user id when logged in, the stored `comment_author_ip` when a guest), on the same post, within **10 minutes**, is **`409 rest_comment_duplicate`**. It runs first because the guard is the one layer whose answer differs between a first and a second submission: had the honeypot come first, a trapped caller would get `201` twice where an honest one gets `201` then `409`, and the trap would announce itself on the second request. For the same reason the guard also remembers what the honeypot (and a `comments:pre_insert` veto) discarded inside the window, so both paths answer the repeat identically. **(4)** The **`_hp` honeypot** — the same hidden-field name the contact-form block uses, so a site has one convention — which is answered with the ordinary `201` success payload while **storing nothing** — the row id included. Emitting `0` there made the trap readable in a single request, and answering with a bare `MAX(comment_id) + 1` made it readable in two: a discard stores nothing, so `MAX` never moves, and two trapped submissions with different bodies came back with the same id — a pair two accepted comments can never produce. The id therefore comes from a per-process high-water mark that is raised to `MAX + 1` on every call and then consumed, so consecutive discards climb exactly as consecutive accepts do and the number is always one no row holds. Nothing is reserved and nothing is written; the residue is that each replica of a multi-node install keeps its own mark, so a bot spreading its probes across nodes can still see a fake id repeat.

> **Anti-spam extension point (`comments:pre_insert`).** An Akismet/Turnstile-style plugin vetoes a comment without a core change by registering `addFilter('comments:pre_insert', fn)`: the callback receives `true` plus a plain object describing the comment about to be written, and returning `false` discards it exactly as a tripped honeypot is discarded (the caller is told nothing), while **throwing** produces a visible refusal with the plugin's own status and `rest_*` code.
...
> **Install token (pre-install setup):** `POST /setup/install` and `POST /setup/test-db` run before the instance is configured (unauthenticated, CSRF-exempt), so they are gated by a **one-time install token** (`backend/src/core/install-token.ts`) — supply it via the `x-install-token` header or an `installToken` body field (constant-time compared; `403` on mismatch). The token is written to a `0600` file at boot (`backend/data/install-token`) and printed in the boot banner **only when stdout is a TTY or `WORDJS_PRINT_INSTALL_TOKEN=1`** — off a TTY (Docker, systemd, CI, anything piped) the banner prints `<siteUrl>/install` with no token in it and names that file to `cat` instead, so the bootstrap secret does not land in whatever aggregates the logs. When it is printed it is a ready-to-open link, `<siteUrl>/install#token=<token>` — in the URL **fragment**, which the browser never transmits and the URL spec strips from `Referer`, so it cannot land in access/proxy logs the way a `?token=` query string would. The install page reads the fragment first, still accepts a legacy `?token=` as a compatibility ramp, and scrubs the token out of the address bar either way. The token is also overridable via the `WORDJS_INSTALL_TOKEN` env var (≥ 16 chars — a shorter value is ignored and a random token generated instead). Both endpoints also early-return `400` once the site is installed, and the on-disk token mirror is cleared after a successful install. `GET /setup/status` is **not** token-gated; `POST /setup/migrate` instead requires admin username/password.
...
| `GET`  | `/api/v1/health/details`| Admin | Full system status: `{ database, mtls, filesystem, sandbox, purge, contentOutbox, timestamp }`. `sandbox.kernel` reports the native Landlock/AppContainer/Seatbelt state, `sandbox.network` the certified network-policy state, and `sandbox.permission` the Node capability floor. `purge.broken` lists the permanent failures that have killed on-demand cache invalidation in this process. `contentOutbox` reports the F3 durable event queue (`pending`, `processing`, `dead`, `oldestDue`, `delayedSeconds`) — `ERROR` on any dead row, `DEGRADED` past 300s of delay. `audit` reports the log-retention job (`retentionDays` the window the last run used, `behind` when it stopped at its row cap with rows still outside the window, `lastRunAt`, `lastRemoved`, `lastError`). See `documentation/deployment.md` |
