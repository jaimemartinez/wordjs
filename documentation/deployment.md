...
> **Latest release:** [v2.1.0](https://github.com/jaimemartinez/wordjs/releases) at `github.com/jaimemartinez/wordjs/releases`.
...
Three further keys switch the native layer itself off, one per OS. Each is **true by default** and disabled only by an explicit `false`, each is a deliberate weakening, and the resulting `disabled` state is exactly what the health surface reports as `sandbox.hardening` / `sandbox.kernel.state` — so with `requireHardening` left at its default, turning one off does not start plugins unconfined, it **refuses** them on that host:

- `sandbox.useKernelHardening` — **Linux**, default `true`. `false` skips the Landlock + seccomp floor entirely, and boot logs `🛡️  Plugin sandbox: kernel hardening DISABLED via config (sandbox.useKernelHardening=false).`
- `sandbox.useAppContainer` — **Windows**, default `true`. `false` skips the AppContainer launch, so an isolated child keeps no package-SID filesystem boundary.
- `sandbox.useSeatbelt` — **macOS**, default `true`. `false` skips the deny-by-default Seatbelt profile.

`sandbox.blockCodeGen` is **default-on** in the same way: each **compiled** child is forked with V8's `--disallow-code-generation-from-strings`, so runtime `eval` / `new Function(string)` is refused at the engine level, underneath the install-time AST scanner that only sees *statically visible* codegen. Set it to `false` only for a plugin whose dependencies genuinely need runtime `Function()`/`eval` (some template engines do); it is force-disabled under `ts-node` regardless, so it only bites a compiled production worker.
