...
5. Cron, cluster coherence, webhooks, then the theme engine (`themeEngine.init()`), then `syncActiveThemeLayout()`, `doAction('init')`, and finally `appReady = true`.

`syncActiveThemeLayout()` (step 5) republishes the **active** theme manifest's `layout` into the `active_theme_layout` option. `switchTheme` already writes it when an admin activates a theme, so this is the reconciliation for every path that changes the layout *without* a switch — a theme update, an edit to `theme.json`, a restore. It is safe on every boot because it is idempotent: it compares in the serialized form and writes only on a real change, which matters because `active_theme_layout` is on the frontend-purge allowlist and a needless write would evict the public cache on each restart. A failure is caught and warned, never fatal.
...
- **Content rendered in Server Components fed server data as props.** The fetched data is passed into `frontend/src/components/public/PostContent.tsx` and `HomeContent.tsx`, which are themselves **Server Components**: the body renders through `frontend/src/components/content/ContentRenderer.tsx`, which dispatches to the shared server-compatible blocks in `content/blocks.tsx`, and only the genuinely interactive pieces (carousels, accordions, tabs, comments) are client islands in that same directory. `sanitize.ts` is a **shared** module (no `"use client"`), so `sanitizeHTML` **is** callable from a Server Component; the `typeof window` branch inside each export picks `sanitize-html` on the server and DOMPurify in the browser. Sanitized-HTML blocks still carry `suppressHydrationWarning` because the two libraries serialize styles slightly differently.
...
**No trust tiers — Android-style per-capability grants (default-deny).** `plugin-trust.ts` was removed: there is no "operator-trusted"/privileged tier and no plugin bypasses the sandbox. **Every** plugin runs in the child process, DB-scoped to its own `wjp_<slug>_` tables with non-secret options only, routes namespaced under `/api/v1/plugin/<slug>`. A plugin's manifest **REQUESTS** capabilities; an admin **GRANTS** each one per plugin in the Plugins UI (`/admin/plugins`), and a bridge capability is allowed only if the manifest requested it **AND** an admin granted it (`src/core/plugin-permissions.ts`, `isGranted`). Grants are stored server-side in the `plugin_grants` option — never self-declarable. No plugin gets raw-HTML hooks (`wordjs_head`/`wordjs_footer`); the host auth JWT cookie (`wordjs_token`) is stripped from forwarded route requests, and dangerous response headers (`Set-Cookie`/CSP/HSTS/`Location`) are stripped from its replies. `fs` read/write is confined to its own directory — **minus** the publicly-served subtree `plugins/<slug>/public/`, which is read-only to the plugin. That carve-out exists because the HTTP surface is the other half of the same rule: `/plugins/<slug>/…` no longer serves the raw plugin tree but an allowlist (`public/**` with a servable extension, plus three fixed host-known files), declared once in `core/io-guard.ts` and consumed by both `index.ts` (may this be served?) and the fs guard (may the plugin write it?). Publishing a writable directory was a write→HTTP-read exfiltration channel that no socket-level control — the `network` grant, the egress guard or the native network boundary — can observe. See `documentation/security.md` §1.3a.

Host-level capabilities each gate on their own grant: becoming the host mail sender (`register-mail-provider`) requires the `email:provider` grant, registering a core notification transport requires `notifications:provider`, and outbound network requires the separate, manifest-independent `network` grant. Grants are seeded at **activation**, for **every** plugin rather than first-party ones only: an admin activating a plugin that holds no grant record grants exactly its **declared** set, persisted only after activation and the AST scan succeed, and only while it has no record — so a later revoke survives re-activation (`backend/src/routes/plugins.ts`). A separate one-time, non-breaking backfill (`backfillActive`) grandfathers plugins that were **already active** at upgrade time so flipping to default-deny doesn't break a running site. Neither path confers privilege, and an admin can revoke. When a plugin's grants change, the child is **hot-reloaded** (`reloadIsolatedPlugin`) so it re-registers routes and re-evaluates host-capability gates (mail/notify providers, network) without a server restart; unload/reload does a full teardown. Teardown splices the plugin's Express layers out by **handler identity**, not by the verb they were registered under (`app.all()` expands to every concrete verb and never leaves a `route.methods.all` key, so verb-keyed unmounting silently left `all` routes mounted on a dead child), and `rpcSend` rejects **immediately** when `postMessage` reports the child is gone instead of waiting out the 30 s RPC timeout. The former `db-migration` plugin has moved into core at `src/core/db-admin/`.

#### Defense in depth

- **AST static scanner at install** (`validatePluginPermissions` in `src/core/plugins.ts`, via acorn): flags `eval`/`Function`/`exec`/`spawn`/`fork`, `require()`/`import()` of sensitive builtins (`child_process`/`worker_threads`/`vm`/…), dynamic `import()`, the `.constructor` Function build, and `process`/`global`/`require` aliasing. **Fail-closed** — an unparseable file is treated as a violation and blocked. The same pass also walks the dependency tree a plugin **ships** (`node_modules/`, previously unread), bounded so a hostile ZIP cannot turn a boot into a multi-minute AST run: at most **4,000** files, **1 MB** per file, **32** directory levels, `.js`/`.cjs`/`.mjs` only, every entry resolved through `realpath` so a symlink pointing outside the plugin directory is skipped **and counted**. Hitting any bound — or an unparseable/unreadable dependency file — is itself reported as a finding rather than passing silently, and a dependency's `require('net')`/`require('dns')` stays a *missing-permission* (fixable in `manifest.json`) rather than a hard block. Runtime containment (`secure-require`, `io-guard`, the worker's ESM hook, `blockCodeGen`) remains the control that actually **stops** a dependency; this pass buys the operator a look. Beyond this static scan there is a **default-on runtime block**: `config.sandbox.blockCodeGen` forks the child with `--disallow-code-generation-from-strings`, hard-blocking `eval`/`new Function(string)` at the V8 level. It is **ON by default** — opt **out** with `config.sandbox.blockCodeGen: false`, only for a plugin whose dependencies genuinely need runtime `Function()`/`eval` (`backend/src/config/app.ts` resolves it as `!== false`; `backend/src/core/plugin-isolate.ts` defaults the local flag to `true`) — and is never applied under ts-node (dev needs codegen to compile TS).
- **In-child runtime guards.** `src/core/secure-require.ts` blocks `worker_threads`/`vm`/`module`/`inspector`/`repl`/`test`/`trace_events`/`cluster`/`async_hooks`/`v8`/`sqlite`/`wasi`/`diagnostics_channel`/`tty`, every `_`-prefixed internal builtin (`_http_agent`, `_http_client`, `_tls_wrap`, `_stream_wrap`, … — they expose socket/stream-wrap primitives below the `net.Socket.prototype.connect` egress chokepoint), `process.binding`/`_linkedBinding`/`getBuiltinModule`, native `.node` addons, and (by default) the network modules `net`/`tls`/`dgram`/`http`/`https`/`http2`/`dns`/`dns/promises`, which are handed out only — in their egress-guarded form — when the `network` grant is present. Every other builtin must appear on an explicit reviewed-safe allowlist; `backend/src/tests/sandbox-builtin-classification.test.ts` fails when a Node upgrade introduces a builtin that lands in no bucket. `src/core/io-guard.ts` blocks `fs` writes to plugin code and reads of `.env`/secret files **and** the live database files (any `.db`/`.sqlite`/`.sqlite3` file plus `-wal`/`-shm`/`-journal` sidecars, e.g. `data/wordjs.db`); reads are confined to the plugin's **own** directory, so a plugin cannot read a **sibling** plugin's source/data/secrets (any `package.json`/`node_modules` path stays readable so module resolution works, but never inside a sibling plugin's dir — IO-1). These run inside the child too, so the plugin's own `fs`/`child_process` are sandboxed even there.
...
- **Preventive (opt-in, Linux):** a cgroup v2 `memory.max` via `systemd-run --user --scope -p MemoryMax=805306368 -p MemorySwapMax=0 -p TasksMax=512` (768 MiB; `-p CPUQuota=N%` is added to the same scope when `config.sandbox.cpuQuotaPercent` is set; no root; `--scope` runs node as a direct child inheriting the IPC fd). Enabled with `config.sandbox.useCgroupMemoryCap=true`; it is probe-gated (validates spawn + IPC + clean teardown of the exact scope on the host before activating) and logs `[Sandbox] preventive cgroup caps ACTIVE (systemd-run --user --scope <props> per isolated child).` — on probe failure it warns `[Sandbox] cgroup caps requested … but the probe failed … falling back to RLIMIT_AS + the RSS poll.` **Default OFF** — auto-detect proved unreliable (some hosts/CI have `systemd-run` but no usable `--user` bus).
- **Reactive CPU watchdog (default ON, non-win32, non-cgroup paths):** the same host-side poll that reads the child's rss also samples its cumulative CPU time (Linux `/proc/<pid>/stat` `utime`+`stime` over the host's real `CLK_TCK`; macOS `ps -o cputime=` in the same `ps` call) and `SIGKILL`s a child holding ≥ 95% of one core for `config.sandbox.cpuBurstSeconds` (default 60, `0` disables) with no quiet tick, recording the reason in `health.lastError`. The decision is the pure `isSustainedCpuBurn(samples, thresholdRatio, windowMs)` so it is unit-tested without burning a core. Skipped on win32 (preventive Job Object CPU rate cap) and in cgroup mode (`child.pid` is `systemd-run`).
...
- **Loose backstop (Linux):** a kernel `RLIMIT_AS` virtual ceiling (default **16384 MB**, override via `config.sandbox.addressSpaceCapMb`) applied through an `sh -c 'ulimit -v N; exec node …'` wrapper that preserves the IPC fd, plus `--max-old-space-size=256` for the JS heap. `RLIMIT_AS` can only be loose because V8's ~4 GB pointer-compression cage counts against it. Darwin's similarly named limit is measured and rejected because current macOS does not enforce it.

Process separation means a child OOM/crash never takes down the host on any platform.

> **Honest residual:** production children are capability-reduced by Landlock+seccomp (Linux), AppContainer (Windows), or Seatbelt (macOS), with a common fail-closed contract. Preventive resident-memory enforcement is still asymmetric: cgroup v2 is opt-in on Linux, Job Objects are default-on on Windows, and macOS has only the V8 heap bound plus reactive RSS enforcement. No independent third-party audit yet. See **POSITIONING.md** section 2 for the canonical posture.
...
- **TOTP two-factor auth** — after password validation an MFA-enabled account completes login by submitting a TOTP or backup code to `POST /api/v1/auth/mfa` (enrol/manage via `/auth/mfa/setup|enable|disable|status|backup-codes`). Enrolment is **sudo-gated**: `/setup` and `/enable` both require `currentPassword`, so a hijacked cookie cannot bind an attacker's authenticator; `POST /users/:id/mfa/reset` (`edit_users`, session-only, never self) is the administrative way out of a lockout. An admin-enforced **MFA-by-role policy** (`GET`/`PUT /auth/mfa/policy` plus a global compliance gate) can require 2FA for chosen roles.

### Cookie writes carry a double-submit CSRF token

Authentication is only half of what a cookie-carrying write must satisfy. `issueSessionCookie()` — the one door every session goes through (login, register, refresh, MFA completion, installer auto-login) — mints a **fresh** `wjs_csrf` cookie beside the `wordjs_token` session cookie, identical in transport except that it is **not** `HttpOnly` so the page can read it. Every `POST`/`PUT`/`PATCH`/`DELETE` under the API prefix must echo it back in an `X-CSRF-Token` header, compared byte-for-byte in constant time; a missing or mismatched header answers **`403 rest_csrf_token`**.

The token gate is **ANDed with** the Origin/Host allow-list, never a replacement for it — a perfect token does not disable the origin check, and a header-less cookie request is still refused with `403 rest_csrf_invalid`. Three things sit outside the token gate: a **Bearer** caller (there is no ambient cookie to ride), a session cookie that no longer **verifies** (`authenticate` answers 401 anyway, so the token would protect nothing — this is what lets a pre-release session upgrade instead of bricking), and the two sign-in steps **`POST /auth/login`** and **`POST /auth/mfa`**, enumerated as exact paths and for POST only, so `/auth/refresh`, `/auth/mfa/setup` and a path that merely starts with `/auth/login` all stay gated. A session that predates the feature is back-filled with a token on its next authenticated request, including one that 401s. The frontend attaches the header through `frontend/src/lib/csrf.ts` (its `api()` client, the XHR upload paths, and the editor presence beacon — which moved from `navigator.sendBeacon`, unable to set headers, to `fetch` with `keepalive`), and `X-CSRF-Token` is an allowed CORS request header so a cross-origin admin's preflight can carry it.
...
- **Tooling & CI:** ESLint (flat config) + Prettier, a `node:test` suite (`src/tests/*.test.ts`, including supertest API integration tests). CI (`.github/workflows/ci.yml`) runs the backend job as **dependency audit (block high/critical prod vulns, `scripts/ci-audit.mjs`) → strict typecheck → lint → build → the F0–F6 phase verifiers (`backend/scripts/verify-f*.ts`) → license gate (block AGPL/SSPL via the pinned `license-checker` devDependency) → tests (`npm test`, with `WORDJS_CI_DB=1` so the Postgres/MySQL conformance blocks hard-fail instead of skipping) → F0 content performance budgets (`npm run perf:f0`) → integration tests against the Postgres + Redis service containers (`npm run test:integration`) → marketplace catalog integrity (`build-marketplace.js` + `verify-marketplace.js --rebuild`)**, alongside jobs for the committed-gate check (`gates-travel`), multi-node coherence (two backends on a shared Postgres + Redis, plus a container-stack parse check), gateway tests, the install channel (`packages/create-wordjs`: audit, tests with a zero-tests guard, and a `--help` boot of the CLI), frontend lint + build (with the token-manifest and asset-version anti-drift gates), a Verso Playwright E2E, and a smoke-boot of the compiled bundle in every mode. Beyond `ci.yml`, a separate `.github/workflows/codeql.yml` runs **CodeQL SAST** (JavaScript/TypeScript) on push/PR to `main` and weekly; `.github/workflows/dependency-audit.yml` re-runs that same `scripts/ci-audit.mjs` gate **daily at 04:41 UTC** (plus on demand) across **all six** workspaces — including `/` and `/setup`, which `ci.yml` never audits — with `fail-fast: false` so every leg reports, and on failure opens or comments on a `Dependency audit failed (<date>)` issue naming the affected workspaces and linking the run; and `.github/workflows/release.yml` gates a tag on `npm run ci:all` + audits + typecheck + tests before it publishes, generates a **CycloneDX SBOM** (`wordjs-sbom.cdx.json`) as a release asset, and smoke-deploys the extracted bundle in every mode it claims to support. Third-party GitHub Actions are pinned to immutable commit SHAs for supply-chain integrity.
