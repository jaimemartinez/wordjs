...
// Verso block for __NAME__ — declared in manifest.frontend.versoComponents ({ "entry": ... }).
//
// EXPORT CONTRACT (detected by frontend/scripts/generate-verso-plugin-registry.js):
//   • single block    → `export const versoComponentDef = {...}` + a default-exported render
//                       component (this template). Registered as "__PASCAL__" in the editor.
//   • multiple blocks → export a const named versoComponents: { BlockName: { ...def, render }, ... }
...
//
// COMPATIBILITY: the historical spellings still work and always will — manifest key
// `frontend.puckComponents`, folder `client/puck/<Pascal>Puck.tsx`, exports `puckComponents` /
// `puckComponentDef`. A plugin published before the editor was renamed keeps loading untouched; the
// build just prints a deprecation line. New plugins should use the Verso names above.
...
interface __PASCAL__VersoProps {
    title?: string;
    body?: string;
    elementId?: string;
}
...
export default function __PASCAL__Verso({ title = "", body = "", elementId = "" }: __PASCAL__VersoProps) {
    return (
