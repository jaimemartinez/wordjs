 ...
 * are worse than one.
 */
const CSS_POSITION_KEYWORDS: ReadonlySet<string> = new Set([
    'static', 'relative', 'absolute', 'fixed', 'sticky',
]);
const MAX_CLASS_ATTR_TOKEN = 64;
...
const POSITION_BINDING_BASE = [
    // GENERATED BY DERIVATION, sorted — see the frontend copy. Do not curate by hand.
...
const MAX_EXTRA_CLASS_TOKENS = 3;
...
const sanitize = (html: string) => {
    return sanitizeHtml(html, withClassBound({
        ...
        // MIRROR of ALLOWED_EMBED_HOSTS (frontend/embed-hosts.js), which is also what the CSP
        // `frame-src` is derived from. `www.youtube-nocookie.com` is what YouTube hands an author who
        // ticks "Enable privacy-enhanced mode" under Share → Embed; the frontend block has always
        // resolved it, and dropping it here silently deleted the iframe from pasted HTML on save.
        // A different package, so it cannot import that module — the two are pinned together by
...
const AUTHOR_CSS_PROPS = new Set([
 ...
 */
const AUTHOR_CSS_VARS = new Set(['--wjs-text-color', '--wjs-heading-color']);
const URL_BEARING_PROP = /^backgroundImage$|-image$/;
/** `;` is the one that mattered: React does not escape it inside a style value. */
const UNSAFE_STYLE_VALUE = /url\(|image-set\(|expression|javascript:|[;{}<>\\@]/i;
...
const NARROWED_VAR_VALUE = new Map<string, (raw: string) => string | null>([
    ['--wjs-pricing-highlight-scale', clampedNumber(SCALE_MIN, SCALE_MAX)],
    ['--wjs-scroll-amt', clampedNumber(-200, 200)],
