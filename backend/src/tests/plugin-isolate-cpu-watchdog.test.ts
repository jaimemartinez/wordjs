 ...
 * "by default a plugin can still burn CPU (DoS)" on every path but Windows. The watchdog in
 * core/plugin-isolate.ts closes that: the host-side poll that already reads a child's rss now also reads
 * its cumulative CPU time, and a child holding >= 95% of ONE core for sandbox.cpuBurstSeconds (default 60)
 * WITHOUT a single quiet tick is SIGKILLed.
 *
