...
/**
 * Puck block "RelatedPosts" — automatic per-post related articles (YARPP / Jetpack parity).
 *
 ...
 *  - In the Puck editor iframe there is no real post URL, so the block renders a hint plus a
...
function extractCategoryNames(post) {
    ...
    try {
        ...
        const puck = meta && meta._puck_data;
        const root = puck && puck.root;
...
export default function RelatedPostsPuck({ title, maxPosts, showImages, showDate, showExcerpt }) {
    ...
    useEffect(() => {
        ...
        // Editor detection: the Puck editor renders blocks inside a same-origin iframe; the
