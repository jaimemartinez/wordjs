...
/**
 * La capa ③ del wrapper — reexportada desde `lib/verso/interactions` para que el wrapper del
 * editor y el del público sigan mirando a UN solo sitio (este fichero), igual que hacen con
 * `hideClasses`/`animClasses`/`appearanceToStyle`.
 *
 * El motor vive en su propio módulo porque es grande y tiene partes (compilador, normalizador,
 * runtime) que este fichero no debe arrastrar; pero la REGLA no cambia: puro, sin `"use client"`,
 * una implementación y dos superficies de render. Dónde encaja la capa y por qué está documentado
 * en la cabecera de `lib/verso/interactions/shell.ts`.
 *
 * `anim` NO se toca: sigue produciendo exactamente las mismas clases y el mismo CSS estático.
 */
export { ixLayer, IX_STATE_ATTR, IX_TRIGGER_ATTR } from "@/lib/verso/interactions";
export type { IxLayer, IxSpec } from "@/lib/verso/interactions";
