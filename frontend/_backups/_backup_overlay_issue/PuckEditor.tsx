...
const IframeContextWrapper = ({ children }: { children: React.ReactNode }) => {
    ...
    useEffect(() => {
        ...
        // Copy styles and patch environment
        const initIframe = () => {
            // Copy <link rel="stylesheet">
            Array.from(parentDoc.querySelectorAll('link[rel="stylesheet"]')).forEach(link => {
                const clonedLink = link.cloneNode(true);
                currentDoc.head.appendChild(clonedLink);
            });

            // Copy <style> tags
            Array.from(parentDoc.querySelectorAll('style')).forEach(style => {
                const clonedStyle = style.cloneNode(true);
                currentDoc.head.appendChild(clonedStyle);
            });

            // Critical: Copy body classes (Next.js fonts, etc)
            currentDoc.body.className = parentDoc.body.className;

            // Copy <meta> tags
            Array.from(parentDoc.querySelectorAll('meta')).forEach(meta => {
                const clonedMeta = meta.cloneNode(true);
                currentDoc.head.appendChild(clonedMeta);
            });

            // Inject Base Tag
            const base = currentDoc.createElement('base');
            base.href = parentDoc.location.origin + '/';
            currentDoc.head.appendChild(base);

            // MONKEY PATCH FETCH
            window.fetch = window.parent.fetch;

            // Sync Auth Token for API calls (Critical for protected menus/settings)
            try {
                if (window.location.origin === parentDoc.location.origin) {
                    const token = window.parent.localStorage.getItem("wordjs_token");
                    if (token) {
                        window.localStorage.setItem("wordjs_token", token);
                    }
                }
            } catch (e) {
                console.warn("Could not sync token", e);
            }
        ...
        const resizeObserver = new ResizeObserver(() => {
            if (window.frameElement) {
                const height = currentDoc.body.scrollHeight;
        ...
        setTimeout(() => {
            if (currentDoc.body) {
                resizeObserver.observe(currentDoc.body);
            }
        }, 100);
    ...
    if (!ready) return <div className="p-4 flex items-center justify-center text-gray-400">Loading editor context...</div>;
...
export default function PuckEditor({
    ...
    // Track current puck data for SSR preview updates
    const [currentPuckData, setCurrentPuckData] = useState<Data>(data);

    useEffect(() => {
        if (initialData && !hasInitializedRef.current) {
            setData(initialData);
            setCurrentPuckData(initialData);
    ...
    useEffect(() => {
        const calculateScale = () => {
            ...
            const containerWidth = containerRef.current.offsetWidth - 64; // 64px padding
            let targetWidth = 0;

            if (viewport === 'mobile') targetWidth = 375;
            else if (viewport === 'tablet') targetWidth = 768;
            else targetWidth = 1280; // Force specific desktop width for scaling logic

            // Scale down if container is smaller than target, otherwise 1 (centered)
                ...
                >
                    ...
                    <div className="flex flex-col h-full w-full overflow-hidden">
                        ...
                        <div className="relative flex-1 w-full bg-slate-100 overflow-hidden">
                            ...
                            {/* MAIN PREVIEW AREA - Offset by sidebar width */}
                            <div
                                id="puck-preview-viewport"
                                className={`h-full overflow-y-auto overflow-x-hidden flex flex-col items-center py-8 relative transition-all duration-300 ${showSidebar ? 'ml-[340px]' : 'ml-0'}`}
                                ref={containerRef}
                            >
                                <div
                                    id="puck-preview-wrapper"
                                    className="transition-all duration-300 ease-in-out min-h-full bg-white shadow-lg overflow-hidden relative"
                                    style={{
                                        width: getViewportWidth(),
                                        minHeight: scale < 1 ? `calc(100% / ${scale})` : '100%',
                                        height: 'auto',
