...
export const RichTextEditor = React.memo(({ value, onChange, transparent = false }: {
    value: string;
    ...
    // ... (saveSelection, restoreSelection, execCmd, changeFontSize, handleMouseDown, ToolbarButton logic)
    ...
    const changeFontSize = (delta: number) => {
        ...
        if (sel && sel.rangeCount > 0 && !sel.isCollapsed) {
            ...
            const computedStyle = anchorEl ? window.getComputedStyle(anchorEl) : null;
            const currentSize = parseInt(computedStyle?.fontSize || '16') || 16;
    ...
    return (
        <div className={`
            ${transparent ? 'relative' : 'border border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white group focus-within:ring-2 focus-within:ring-blue-100 focus-within:border-blue-400 transition-all duration-300'}
        `}>
            {/* Modern Toolbar */}
            <div className={`
                flex flex-wrap items-center gap-1 p-2 border-gray-100
            ...
            `}>
                ...
                <div className="flex bg-white rounded-lg border border-gray-100 p-0.5 shadow-sm items-center gap-1">
                    <ToolbarButton icon="fa-minus" onClick={() => changeFontSize(-2)} title="Reducir" />
                    <div className="relative group/input">
                        <input
                            type="number"
                            className="w-10 text-center text-xs font-medium text-gray-600 border-none bg-transparent focus:ring-0 p-0"
                            placeholder="16"
                            disabled
                        />
                        <span className="absolute inset-0 flex items-center justify-center text-xs text-gray-400 pointer-events-none">Px</span>
                    </div>
                ...
                <div className="flex bg-white rounded-lg border border-gray-100 p-0.5 shadow-sm">
                    <div className="relative w-8 h-8 flex items-center justify-center hover:bg-gray-50 rounded cursor-pointer group/color">
                ...
                <div className="flex bg-white rounded-lg border border-gray-100 p-0.5 shadow-sm">
                    <ToolbarButton icon="fa-fill-drip" onClick={cycleEditorBg} title="Cambiar Fondo Editor" />
                </div>
                ...
                <ToolbarButton icon="fa-eraser" onClick={() => execCmd('removeFormat')} title="Limpiar formato" />
            </div>
            ...
            <div
                ...
                className="p-4 min-h-[160px] max-h-[400px] overflow-y-auto focus:outline-none text-gray-700 leading-relaxed text-sm selection:bg-blue-100 transition-colors duration-200"
...
const baseConfig = {
    ...
    components: {
        ...
        Text: {
            category: "content",
            fields: {
                // Content is edited inline only - no properties panel editor
                elementId: { type: "text", label: "ID / Ancla (opcional)" },
            },
            defaultProps: {
        ...
        Columns: {
            ...
            render: ({ distribution, gap, minHeight, backgroundColor, borderRadius, columnStyles, elementId }: any) => {
